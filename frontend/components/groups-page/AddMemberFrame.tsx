...
import { AddUserToGroupFormData, PublicUserDisplayData } from "@/schemas/fe.schema";
import Image from "next/image"

export default function AddMemberFrame({ groupId,groupMembers }: { groupId: number,groupMembers:PublicUserDisplayData[] | undefined}) {
	return (
		<div className="flex flex-row">
			<div className="basis-1/3">
				<ViewGroupMembersFrame groupMembers={groupMembers} />
			</div>
...
function ViewGroupMembersFrame({ groupMembers }: { groupMembers: PublicUserDisplayData[] | undefined }) {
	return (
		<div className="m-auto h-max max-w-md space-y-8 pl-7">
			<div className="text-center">
				<h2 className="mt-6 text-3xl font-extrabold text-gray-900">
					 Group Members
				</h2>
			...
			{groupMembers?.map((member) => (
				<div 
				key={member.username}
				className="mx-auto flex items-center gap-x-4 rounded-l bg-white p-3  outline outline-black/10 ">
				<Image src="/profile-placeholder.png" className="rounded-full" width={48} height={48} alt="avatar" ></Image>
				<div>
					<div className="text-xl font-medium text-black ">{member.username}</div>
				</div>
				</div>
